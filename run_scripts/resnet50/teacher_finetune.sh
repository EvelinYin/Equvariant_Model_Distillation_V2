...
CUDA_VISIBLE_DEVICES=0,1,2,3 python main.py \
  --config src/exp_configs/vit/teacher_finetune.yaml \
  train_teacher=true \
  teacher_model.model_structure="pretrained_resnet50" \
  teacher_model.resnet50_config.model_name="microsoft/resnet-50" \
  teacher_train.epochs=300 \
  teacher_train.learning_rate=1e-5 \
  teacher_train.scheduler_warmup_epochs=15 \
  teacher_train.weight_decay=5e-2 \
  teacher_train.group="Rot90Group" \
  data.batch_size=256 \
  logging.wandb_name="resnet_teacher_rot90aug_resnet50_weight_selection" \
  logging.wandb_mode="online" \